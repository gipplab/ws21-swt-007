...
public class JoinOnlinePageController {
	...
	public void JoinButtonOnClick(ActionEvent event) throws IOException {
	    // ueberpruefen ob der Name der Spieler leer oder '-' beinhaltet
			...
			TablePosition pos = hostTableView.getSelectionModel().getSelectedCells().get(0);// get selected cell in table
			int row = pos.getRow();
			...
			String hostToJoin = (String) col.getCellObservableValue(hostTableView.getItems().get(row)).getValue();// this gives the value in the selected cell
			...
			resp = Client.accessServer(msg);// send the server the pseudoname and the selected room
			if(resp.equals("Added")) // When the player has added, show him the waiting scenes
			{
	...
	//Fill in the table with the names of the rooms and the number of players in each room
    ...
    // funktion zu ueberpruefen ob der Name der Spieler leer oder '-' beinhaltet
